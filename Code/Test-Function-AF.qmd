...
write.csv(data.frame(a = 9:11, b = 12:14), "temp/case/A005_SD020/222222/file3.csv")
...
# It also allows for a 'master' CSV to be writen as part of the function as well, but for directly using the function in R there is no need to write the csv, so that's why it is a Boolean value
# NOTE: currently the strings the loop looks for are hard coded, but this code can be generalized if need be. 
